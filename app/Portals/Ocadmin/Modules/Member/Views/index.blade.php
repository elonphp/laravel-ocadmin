    ...
    <div class="page-header">
        <div class="container-fluid">
            <div class="float-end">
                ...
                <a href="{{ route('lang.ocadmin.member.create') }}" data-bs-toggle="tooltip" title="{{ $lang->button_add }}" class="btn btn-primary">
...
$(document).ready(function() {
    ...
    $('#button-filter').on('click', function() {
        var url = '{{ route('lang.ocadmin.member.list') }}?';
        var params = [];
    ...
    $('#button-clear').on('click', function() {
        ...
        var url = '{{ route('lang.ocadmin.member.list') }}';
        window.history.pushState({}, null, '{{ route('lang.ocadmin.member.index') }}');
        $('#member-list').load(url);
    });
    ...
    $('#button-delete').on('click', function() {
        ...
        if (confirm('{{ $lang->text_confirm_batch_delete }}'.replace('%s', selected.length))) {
            $.ajax({
                url: '{{ route('lang.ocadmin.member.batch-delete') }}',
                type: 'POST',
