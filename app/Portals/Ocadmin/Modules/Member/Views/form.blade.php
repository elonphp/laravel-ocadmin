...
<div id="content">
    <div class="page-header">
        <div class="container-fluid">
            <div class="float-end">
                ...
                <a href="{{ route('lang.ocadmin.member.index') }}" data-bs-toggle="tooltip" title="{{ $lang->button_back }}" class="btn btn-secondary">
    ...
    <div class="container-fluid">
        <div class="card card-default">
            ...
            <div class="card-body">
                <form action="{{ $user->exists ? route('lang.ocadmin.member.update', $user) : route('lang.ocadmin.member.store') }}" method="post" id="form-member" data-oc-toggle="ajax">
                    @csrf
                    @if($user->exists)
                    @method('PUT')
                    @endif
                    ...
                    @if(!empty($hasAdminRole))
                    <div class="row mb-3">
                        <label class="col-sm-2 col-form-label">{{ $lang->column_password }}</label>
                        <div class="col-sm-10">
                            <div class="alert alert-warning mb-0">
                                <i class="fa-solid fa-triangle-exclamation"></i> {{ $lang->help_password_admin }}
                            </div>
                        </div>
                    </div>
                    @else
