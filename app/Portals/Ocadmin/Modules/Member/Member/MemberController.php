...
class MemberController extends OcadminController
{
    ...
    public function create(Request $request): View
    {
        ...
        $data['back_url'] = $this->backUrl('lang.ocadmin.member.members.index', $request);
    ...
    public function batchDelete(Request $request): JsonResponse
    {
        ...
        // 排除擁有後台角色的使用者（依 config('portals.ocadmin.role_prefix') 過濾）
        $rolePrefix = config('portals.ocadmin.role_prefix');
        $backendUserIds = User::whereIn('id', $ids)
            ->whereHas('roles', fn($q) => $q->where('name', 'like', $rolePrefix . '.%'))
            ->pluck('id')
            ->toArray();
