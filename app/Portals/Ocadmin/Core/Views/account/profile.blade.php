    ...
    <div class="container-fluid">
        <div class="card card-default">
            <div class="card-header">
                <i class="fa-solid fa-user-circle"></i> {{ $lang->text_edit }}
            </div>
            <div class="card-body">
                <ul class="nav nav-tabs">
                    <li class="nav-item"><a href="#tab-data" data-bs-toggle="tab" class="nav-link active">{{ $lang->tab_data }}</a></li>
                    <li class="nav-item"><a href="#tab-password" data-bs-toggle="tab" class="nav-link">{{ $lang->tab_password }}</a></li>
                </ul>
                <form action="{{ route('lang.ocadmin.account.profile.update') }}" method="post" id="form-profile" data-oc-toggle="ajax">
                    ...
                    <div class="tab-content">
                        ...
                        <div id="tab-data" class="tab-pane active">
                            ...
                            {{-- 唯讀：角色 --}}
                            <div class="row mb-3">
                                <label class="col-sm-2 col-form-label">{{ $lang->column_roles }}</label>
                                <div class="col-sm-10">
                                    <div class="form-control-plaintext">
                                        @forelse($user->roles as $role)
                                        <span class="badge bg-secondary me-1">{{ $role->display_name }}</span>
                                        @empty
                                        <span class="text-muted">{{ $lang->text_none }}</span>
                                        @endforelse
                                    </div>
                                    <div class="form-text">{{ $lang->help_roles_readonly }}</div>
                                </div>
                            </div>
                        </div>
