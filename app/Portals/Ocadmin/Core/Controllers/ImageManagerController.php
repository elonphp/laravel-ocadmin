...
/**
 * 圖片管理器（Image Browser Modal）— picker
 *
 * 處理圖片瀏覽、上傳、刪除、建立資料夾。只操作檔案系統，不涉及資料庫。
 *
 * 操作範圍嚴格限定 picker root：storage/app/public/images/。
 * system-managed 子樹（products/、banners/ 等，由業務模組依 owner row 強制管理）
 * 與此平行隔離，picker 看不到也碰不到。
 *
 * 回傳給表單的 path 含完整 'images/' 前綴（相對 storage/app/public/，
 * asset('storage/' . $path) 直接可用）。
 */
class ImageManagerController extends OcadminController
{
    protected function setLangFiles(): array
    {
        return ['common/image-manager'];
    }
    ...
    public function index(Request $request): string
    {
        ...
        return view('ocadmin::common.image-manager.index', $data)->render();
    }
    ...
    public function list(Request $request): string
    {
        ...
        $base = storage_path('app/public/images/');
        ...
        foreach (array_slice($paths, $start, $limit) as $path) {
            ...
            $name = basename($path);
            $relativePath = 'images/' . mb_substr($path, mb_strlen(str_replace('\\', '/', $base)));
        ...
        return view('ocadmin::common.image-manager.list', $data)->render();
    }
    ...
    public function delete(Request $request): JsonResponse
    {
        ...
        // $base 用 app/public/ 是為了 resolve frontend 傳回的相對路徑（如 'images/foo.jpg'）；
        // 但實際刪除操作必須限制在 picker root（images/）內，不能波及 products/ / banners/
        // 等 system-managed root。
        $base = str_replace('\\', '/', storage_path('app/public/'));
        $pickerRoot = str_replace('\\', '/', storage_path('app/public/images/'));

        $paths = $request->input('path', []);

        // 驗證所有路徑：必須在 picker root 內且不可為 picker root 本身
        foreach ($paths as $path) {
            $path = html_entity_decode($path, ENT_QUOTES, 'UTF-8');
            $realPath = str_replace('\\', '/', realpath($base . $path));

            if (!$realPath || $realPath . '/' === $pickerRoot || !str_starts_with($realPath . '/', $pickerRoot)) {
                $json['error'] = $this->lang->error_directory;
                break;
            }
        }
