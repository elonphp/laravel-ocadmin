...
<div id="content">
    <div class="page-header">
        <div class="container-fluid">
            ...
            <h1>{{ $group->exists ? $lang->text_edit : $lang->text_add }}</h1>
        </div>
    </div>

    <div class="container-fluid">
        <div class="card">
            <div class="card-header"><i class="fa-solid fa-pencil"></i> {{ $group->exists ? $lang->text_edit : $lang->text_add }}</div>
            <div class="card-body">
                <form id="form-group" action="{{ $save_url }}" method="post" data-oc-toggle="ajax">
                    @csrf
                    @if($group->exists)
                    @method('PUT')
                    @endif
                        ...
                        <table id="level-table" class="table table-bordered table-hover">
                            ...
                            <tbody id="level-tbody">
                                ...
                                <tr id="level-row-{{ $index }}" data-index="{{ $index }}">
                                    ...
                                    <td class="text-end">
                                        <button type="button" onclick="removeLevel({{ $index }});" data-bs-toggle="tooltip" title="{{ $lang->button_delete }}" class="btn btn-danger btn-sm"><i class="fa-solid fa-minus-circle"></i></button>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
                            <tfoot>
                                <tr>
                                    <td colspan="2"></td>
                                    <td class="text-end">
                                        <button type="button" onclick="addLevel();" data-bs-toggle="tooltip" title="{{ $lang->button_add }}" class="btn btn-primary btn-sm"><i class="fa-solid fa-plus-circle"></i></button>
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
...
function addLevel() {
    ...
    html += '<td class="text-end"><button type="button" onclick="removeLevel(' + level_row + ');" data-bs-toggle="tooltip" title="{{ $lang->button_delete }}" class="btn btn-danger btn-sm"><i class="fa-solid fa-minus-circle"></i></button></td>';
    html += '</tr>';
