        ...
        <div class="card card-default">
            ...
            <div class="card-body">
                <form action="{{ $save_url }}" method="post" id="form-setting" data-oc-toggle="ajax">
                    ...
                    @php $multiLocale = count(config('localization.supported_locales', [])) > 1; @endphp
                    <div class="row mb-3">
                        <label for="input-name" class="col-sm-2 col-form-label">{{ $lang->column_name }}</label>
                        <div class="col-sm-10">
                            <input type="text" name="name" value="{{ old('name', $setting->name) }}" placeholder="{{ $multiLocale ? '預設名稱（各語言共用）' : '名稱' }}" id="input-name" class="form-control">
