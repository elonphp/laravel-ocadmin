...
$(document).ready(function() {
    ...
    var exportAllUrl = '{{ $export_all_url }}';
    ...
    $('#button-sync-confirm').on('click', function() {
        ...
        var syncUrl = '{{ $sync_url }}'.replace('__TABLE__', currentDiffTable);
        ...
        $.ajax({
            ...
            dataType: 'json',
            beforeSend: function() {
                $('#button-sync-confirm').prop('disabled', true).html('<i class="fa-solid fa-spinner fa-spin"></i> 同步中...');
            },
            success: function(json) {
                if (json.success) {
                    alert(json.message);
            ...
            complete: function() {
                $('#button-sync-confirm').prop('disabled', false).html('<i class="fa-solid fa-sync"></i> {{ $lang->button_sync }}');
            }
        });
    });
