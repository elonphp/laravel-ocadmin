...
<div id="content">
    <div class="page-header">
        <div class="container-fluid">
            <div class="float-end">
                <button type="button" data-bs-toggle="tooltip" title="{{ $lang->button_filter }}" onclick="$('#filter-organization').toggleClass('d-none');" class="btn btn-light d-lg-none">
                    <i class="fa-solid fa-filter"></i>
                </button>
                <a href="{{ route('lang.ocadmin.organizations.create') }}" data-bs-toggle="tooltip" title="{{ $lang->button_add }}" class="btn btn-primary">
    ...
    <div class="container-fluid">
        <div class="row">
            {{-- 篩選區塊 --}}
            <div id="filter-organization" class="col-lg-3 col-md-12 order-lg-last d-none d-lg-block mb-3">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form id="form-filter">
                            ...
                            <div class="mb-3">
                                <label class="form-label">{{ $lang->column_business_no }}</label>
            ...
            <div class="col-lg-9 col-md-12">
                <div class="card">
                    ...
                    <div id="organization-list" class="card-body">
                        {!! $list !!}
                    </div>
                </div>
            </div>
...
$(document).ready(function() {
    // AJAX 分頁 & 排序
    $('#organization-list').on('click', 'thead a, .pagination a', function(e) {
        e.preventDefault();
        $('#organization-list').load($(this).attr('href'));
    });

    // 篩選
    $('#button-filter').on('click', function() {
        var url = '{{ route('lang.ocadmin.organizations.list') }}?';
        var params = [];
    ...
    $('#button-clear').on('click', function() {
        ...
        var url = '{{ route('lang.ocadmin.organizations.list') }}';
        window.history.pushState({}, null, '{{ route('lang.ocadmin.organizations.index') }}');
        $('#organization-list').load(url);
    });
    ...
    $('#button-delete').on('click', function() {
        ...
        if (confirm('{{ $lang->text_confirm_batch_delete }}'.replace('%s', selected.length))) {
            $.ajax({
                url: '{{ route('lang.ocadmin.organizations.batch-delete') }}',
                type: 'POST',
