...
### 啟用前提：token-based auth_driver

UserDevice 模組的本質是「裝置 token 註冊 + 撤銷」。session-based auth 沒有「裝置」概念（cookie 是 browser session 不是 device），所以 UserDevice 模組啟用需：

- `config/portals.php` 的 `ocadmin.auth_driver` 必須 ∈ `{sanctum, oauth}`
- `auth_driver` 為其他值（未來若有 session）時 UserDevice 模組不該啟用，否則裝置 token 無從綁定

非強制 2FA — 2FA 是登入額外驗證 challenge，跟裝置管理邏輯獨立（見下「與 2FA 的關係」）。
...
| `app/Portals/Ocadmin/Modules/Account/UserDeviceController.php` | 個人裝置管理（我的裝置） |
| `app/Portals/Ocadmin/Core/Controllers/System/Acl/UserDeviceController.php` | 系統管理（所有人的裝置） |
...
```
Modules/Account/                                  ← 使用者管自己的裝置
└── UserDeviceController.php                     我的裝置列表、遠端登出、撤銷信任

Core/Controllers/System/Acl/                      ← 管理員管所有人的裝置
└── UserDeviceController.php                     檢視任意使用者裝置、強制登出
```

> routes 內兩支 controller 同名，靠 import alias 區分：Core/ 不 alias，Modules/ 加 `as AccountUserDeviceController`（議題 3 §撞名處理範式）。
