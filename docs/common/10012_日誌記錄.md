...
使用 `RequestLog` 而非 `Log`，避免與 Laravel 內建的 `Illuminate\Support\Facades\Log`（用於 `Log::info()` 等）混淆。在同一個檔案中可安全地同時使用兩者：
...
此 Model 命名與 URL / Permission / Controller 等其他層的 `log` token 分歧，屬 [10023 §9.4.6 Model 命名與外部 namespace 衝突](10023_英文名稱單複數規範.md) 列舉的設計例外。
...
- [10001_Portal概述.md](10001_Portal概述.md)
