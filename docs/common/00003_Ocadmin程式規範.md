...
| 觸發條件 | 範例 |
|---------|------|
| 兩個以上 Portal 共用同一業務動作 | Ocadmin 與 POS Portal 都能建單 → `OrderService::create()` |
...
|------|---------|-----------|---------|
| Core | 手動註冊 | `ocadmin::` | `view('ocadmin::system.acl.permission.index')` |
...
class OcadminController extends BaseController
{
    ...
    /**
     * 子類別回傳模組相對路徑（不含 admin/ 前綴），base controller 會自動加 admin/。
     * 例：子類別回 ['system/acl/permission'] → 載入 lang/{locale}/admin/acl/permission.php
     */
    protected function getLang(string|array $groups): void
    {
        if (!isset($this->lang)) {
            $groups = array_map(fn ($g) => "admin/{$g}", (array) $groups);
...
子類別覆寫 `setLangFiles()` 指定模組語言檔（**回相對路徑、不含 `admin/` 前綴**，由 base controller 自動加上）：
...
| 解析 | 結果 |
|------|------|
| 子類別回傳 | `['system/acl/permission']` |
| base controller 加前綴後 | `['admin/acl/permission']` |
| 對應檔案 | `lang/{locale}/admin/acl/permission.php` |
| `defaultGroup` | `admin/default`（自動載入，提供按鈕/欄位/共用文字，可被模組翻譯覆蓋同名 key） |
...
lang/
├── zh_Hant/                       # 繁體中文
│   └── admin/                     # 後台 Portal 全部語系檔放這
│       ├── default.php            # 共用翻譯（按鈕、欄位、通用文字、tab_basic / tab_trans）
│       ├── acl/
│       │   └── permission.php     # 權限管理
│       └── config/
│           ├── taxonomy.php       # 分類管理
│           └── term.php           # 詞彙管理
└── en/                            # English（部分模組已補；其他逐步補完）
    └── admin/
        └── default.php
```

> 歷史備註：早期文件曾規劃 `lang/{locale}/ocadmin/...` 命名空間，後改為 `admin/`
> 與 base controller 的 `defaultGroup: 'admin/default'` 對齊；目前所有衍生 branch 皆走 `admin/`。

### 語言檔命名規範

| 類型 | 路徑 | `setLangFiles()` 回傳 |
|------|------|------|
| 共用（預設自動載入） | `lang/{locale}/admin/default.php` | — |
| 模組 | `lang/{locale}/admin/{category}/{module}.php` | `['{category}/{module}']` |
...
public function index(Request $request): View
{
    ...
    return view('ocadmin::system.acl.permission.index', $data);
}

// ✗ 不使用 compact()
return view('ocadmin::system.acl.permission.index', compact('permissions'));
```
...
> **2026-05 起全 Ocadmin 取消麵包屑**。`OcadminController` 不再提供 `setBreadcrumbs()` /
> `$breadcrumbs`，新模組不需要實作；既有模組陸續清掉 view 內的 `$breadcrumbs` 引用。

### 為何取消

| 理由 | 說明 |
|---|---|
| 資訊重複 | 側邊選單已表達層級、頁首標題已說明當前位置、瀏覽器 tab title 也有；麵包屑是第三層重複 |
| 維護成本對效益不划算 | 每個 controller 要寫 setBreadcrumbs、改路由名要連帶改、要翻譯 |
| 小螢幕劣勢 | 手機/平板擠不下，要寫 `d-none d-md-block` 之類條件渲染，徒增複雜度 |
| 現代後台趨勢 | Notion / Linear / Vercel / Stripe / GitHub web 等現代 SaaS 後台都不放麵包屑；AdminLTE 4 雖支援但非預設樣式 |
| 已是團隊共識 | ocadmin 與所有衍生專案 codebase **零**個 `setBreadcrumbs` 引用即為證據 |

### 歷史寫法（僅供尚未清理的舊 view 參考，**不要新增**）

```php
// 舊：OcadminController 曾有此方法
...
protected function setBreadcrumbs(): void
{
    $this->breadcrumbs = [
        ...
        (object)['text' => $this->lang->heading_title, 'href' => route('lang.ocadmin.system.acl.permissions.index')],
    ];
}
```
...
namespace App\Portals\Ocadmin\Core\Controllers\System\Acl;
...
class PermissionController extends OcadminController
{
    ...
    public function index(Request $request): View
    {
        ...
        $data['list_url']         = route('lang.ocadmin.system.acl.permissions.list');
        $data['index_url']        = route('lang.ocadmin.system.acl.permissions.index');
        $data['add_url']          = route('lang.ocadmin.system.acl.permissions.create');
        $data['batch_delete_url'] = route('lang.ocadmin.system.acl.permissions.batch-delete');
    ...
    protected function getList(Request $request): string
    {
        ...
        $permissions->withPath(route('lang.ocadmin.system.acl.permissions.list'));
        ...
        $baseUrl = route('lang.ocadmin.system.acl.permissions.list');
        ...
        return view('ocadmin::system.acl.permission.list', $data)->render();
    }
    ...
    public function create(): View
    {
        ...
        $data['save_url'] = route('lang.ocadmin.system.acl.permissions.store');
        $data['back_url'] = route('lang.ocadmin.system.acl.permissions.index');
    ...
    public function edit(Permission $permission): View
    {
        ...
        $data['save_url'] = route('lang.ocadmin.system.acl.permissions.update', $permission);
        $data['back_url'] = route('lang.ocadmin.system.acl.permissions.index');
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'replace_url' => route('lang.ocadmin.system.acl.permissions.edit', $permission),
            'form_action' => route('lang.ocadmin.system.acl.permissions.update', $permission),
        ]);
    }
...
| 同義欄位合併查詢 | 一個輸入框查多欄（如 `filter_phone` = mobile OR telephone） | controller 內展開為 `where(fn($q) => $q->orWhere(...)->orWhere(...))` |

> ⚠️ **手動處理時也要用 `OrmHelper::filterOrEqualColumn`，不要寫 inline `where('col', 'like', "%$v%")`**：
> - `filterOrEqualColumn` 支援 `=` / `<>` / `<` / `>` / `*foo` / `foo*` / 預設 REGEXP 等運算符；inline `like` 全部失效
> - 與 `prepare()` 自動處理 `filter_*/equal_*` 行為一致，使用者體驗統一
> - 處理完手動展開的欄位後 `unset($filter_data['xxx'])` 避免 `prepare()` 重複處理
...
- [ ] 覆寫 `setLangFiles()` 指定模組語言檔（`['xxx/yyy']`，base controller 自動加 `admin/` 前綴）
...
- [ ] 建立模組語言檔 `lang/zh_Hant/admin/{category}/{module}.php`
- [ ] 共用翻譯放 `lang/zh_Hant/admin/default.php`（自動載入），模組專屬翻譯放模組語言檔
...
- v1.8（2026-05-02）：對齊 ocadmin 與衍生專案的程式現況。
  - **麵包屑全 Ocadmin 棄用**：`OcadminController` 不再提供 `setBreadcrumbs()` / `$breadcrumbs`，新模組不需實作；附「為何取消」5 點理由
  - **語言檔路徑改為 `lang/{locale}/admin/...`**：對齊 base controller 的 `admin/` 前綴與 `admin/default` 預設群組（早期 `ocadmin/...` 規劃已棄用）
  - `setLangFiles()` 子類別只回模組相對路徑、不含 `admin/` 前綴
  - OrmHelper 章節加禁戒：手動展開查詢欄位時也要用 `filterOrEqualColumn`，不要寫 inline `where('col', 'like', '%v%')`
