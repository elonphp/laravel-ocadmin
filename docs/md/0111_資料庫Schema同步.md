...
>
> 參考東莞宏力人事系統（出勤資料項目、薪資資料項目可由後台使用者自定義），提供後台介面讓略懂資料庫語言的資訊人員，在交案後於現有程式架構上增減欄位、撰寫 SQL。
>
> **左側選單不顯示此功能，僅 super_admin 可存取。** 使用風險詳見文末「[問題反思](#問題反思)」。
>
> 開發人員於開發階段或交案後客製調整，請使用 [0121_資料庫Transition變更機制](0121_資料庫Transition變更機制.md)。
...
需要搬移資料或變更結構（如欄位改名）時，請使用開發人員專用的 [0121_資料庫Transition變更機制](0121_資料庫Transition變更機制.md)（`php artisan db:transition`）。

本機制（0111）僅處理透過後台 UI 進行的宣告式結構同步。
...
3. 選擇表 → 編輯欄位 → 比對預覽 → 確認執行
```

系統自動比對 schema 定義檔與實際 DB，產生並執行 ALTER SQL。
...
| `app/Portals/Ocadmin/Core/ViewComposers/MenuComposer.php` | 系統管理選單**不顯示**此項目（僅 super_admin 直接存取路由） |
...
類似宏力人事管理系統的「自訂欄位」功能（人事附加資料、考勤計算項目、職級薪資項目），真正需要的是**應用層的動態欄位管理**（EAV 模式或 JSON 欄位），而非 DDL 層級的 ALTER TABLE。本系統相關的薪資模組已在 UI 動態增減薪資項目，走的正是應用層方案，不需要本機制。
...
**上述風險已知悉，本機制保留供發案方資訊人員使用，僅限 super_admin 存取。**

---

**文件版本**: 4.0
**建立日期**: 2026-02-08
**更新日期**: 2026-03-24（定位為發案方工具，左側選單隱藏，僅 super_admin；開發人員改用 0121）
