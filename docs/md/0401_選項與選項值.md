...
- `ctl_products` 是電商商品的本體，自帶 SKU、名稱、價格等，**不需依賴 `inv_items`**
- `inv_items` 是庫存/進銷存的料件本體，**需要庫存管理時才建立**
- 當同一品項既銷售又需庫存管理時，透過 nullable FK 關聯（後期加入）
- 不對外銷售的料件/食材只存在於 `inv_items`，不建 `ctl_products`
...
| `ctl_` | 商品型錄 | 銷售商品、分類、選項、屬性、品牌 — 面向客戶 | `ctl_products`, `ctl_options`, `ctl_categories` |
...
Phase 1：純電商（目前階段）
─────────────────────────────────────────────
Migration 順序（MM=20, Catalog 模組）：

  000001  ctl_options / ctl_option_translations       ← 選項基本資料（先定義）
          ctl_option_values / ctl_option_value_translations
  000200  ctl_products / ctl_product_translations      ← 商品主體
  000302  ctl_product_terms                            ← 關聯表：商品 × 詞彙
  000310  ctl_product_options / ctl_product_option_values ← 關聯表：商品 × 選項

  → 基本資料（000001-000199），商品主體（000200-000299），關聯表（000300+）
  → 完全不依賴 inv_，直接上線銷售
...
inv_movements             ← 進/出/調撥/盤點紀錄
  → ctl_products 加 nullable FK：inv_item_id → inv_items.id
...
| 場景 | 使用的表 | 說明 |
|------|---------|------|
| 純電商賣商品 | `ctl_*` | `ctl_products` 自己就是商品，不需 inv_ |
| 電商 + 庫存追蹤 | `ctl_*` + `inv_*` | `ctl_products.inv_item_id` → `inv_items` |
| 純進銷存（不對外銷售） | `inv_*` + `pur_*` | 不需 ctl_，料件只在 inv_items |
| 既採購又銷售 | 三者都有 | `inv_items` 一筆，ctl_ 和 pur_ 各自 FK 指過去 |

> **選項（options）是純粹的型錄概念**，定義產品可選的規格維度，獨立存在，
> 因此使用 `ctl_` 前綴。不需等 `inv_items` 建立即可使用。
...
### ctl_option_value_translations（選項值翻譯表）
...
```
ctl_options: { id: 1, type: 'select', sort_order: 0 }
ctl_option_translations:
...
```
ctl_options: { id: 2, type: 'radio', sort_order: 1 }
ctl_option_translations:
...
Route name prefix: lang.ocadmin.catalog.options.
...
刪除選項前需檢查是否有產品正在使用此選項（未來 `ctl_product_options` 表建立後）。
