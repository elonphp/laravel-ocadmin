...
- `upload()`：驗證副檔名（ico/jpg/jpeg/png/gif/webp）、MIME 類型、檔名長度（4-255）、上傳大小（`config_file_max_size` MB）
...
**`saveOptionValues()` 方法**：已包含 `'image'` 欄位，無需額外修改

```php
$optionValue = $option->optionValues()->create([
    'code' => $valueData['code'] ?? null,
    'image' => $valueData['image'] ?? null,      // ← image 欄位
    'sort_order' => $valueData['sort_order'] ?? $index,
]);
```

**驗證規則**：加入 `'option_value.*.image' => 'nullable|string|max:255'`
...
6. 儲存表單 → 確認 `clg_option_values.image` 欄位正確寫入
...
*文件版本：v1.1*
*建立日期：2026-02-18*
*更新日期：2026-02-21 — 語系檔與實作對齊、補充 Grid/List 雙視圖說明、修正 form key 為 option_value*
