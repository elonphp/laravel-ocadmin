...
**前置依賴**：`0301 選項與選項值` 模組（`ctl_options`、`ctl_option_values` 表已存在）。
...
### ctl_product_translations（商品翻譯表）

| 欄位             | 類型            | 說明               |
|------------------|-----------------|--------------------|
| id               | bigint PK       | 主鍵               |
| product_id       | bigint FK       | 關聯 ctl_products.id |
...
### ctl_product_options（商品選項關聯表）

| 欄位              | 類型            | 說明                                  |
|-------------------|-----------------|---------------------------------------|
| id                | bigint PK       | 主鍵                                  |
| product_id        | bigint FK       | 關聯 ctl_products.id                  |
| option_id         | bigint FK       | 關聯 ctl_options.id                   |
...
> **用途**：記錄「這個商品使用了哪些選項」。
> 選擇型（select/radio/checkbox）的具體值放在 `ctl_product_option_values`；
> 輸入型（text/textarea/file/date/time/datetime）的值放在 `value` 欄位。

### ctl_product_option_values（商品選項值表）

| 欄位                   | 類型              | 說明                           |
|------------------------|-------------------|--------------------------------|
| id                     | bigint PK         | 主鍵                           |
| product_option_id      | bigint FK         | 關聯 ctl_product_options.id    |
| product_id             | bigint FK         | 關聯 ctl_products.id（冗餘，加速查詢） |
| option_id              | bigint FK         | 關聯 ctl_options.id（冗餘）     |
| option_value_id        | bigint FK         | 關聯 ctl_option_values.id      |
...
```
ctl_products
├── ctl_product_translations       (1:N, by locale)
├── ctl_product_options            (1:N)
│   ├── → ctl_options              (N:1, 引用選項定義)
│   └── ctl_product_option_values  (1:N)
│       └── → ctl_option_values    (N:1, 引用選項值定義)
```
...
```
ctl_products: { id: 1, model: 'TS-001', price: 590.0000, quantity: 100, status: 1 }
ctl_product_translations:
...
database/migrations/
└── 0001_01_20_000002_create_ctl_product_tables.php
...
Route name prefix: lang.ocadmin.catalog.products.
...
Product
  ...
  └── productOptions()    → HasMany ProductOption
        ├── option()             → BelongsTo Option (ctl_options)
        └── productOptionValues() → HasMany ProductOptionValue
              └── optionValue()        → BelongsTo OptionValue (ctl_option_values)
```
...
- **搜尋選項**：autocomplete 搜尋 `ctl_options`，選擇後添加到頁面
- 每個已添加的選項顯示為一個 **fieldset**：
  ...
  - **根據選項類型**：
    - 選擇型（select/radio/checkbox）：顯示選項值表格
      ...
      - 新增按鈕可從該選項的 `ctl_option_values` 添加值
...
1. 驗證商品主表欄位 + 多語翻譯
2. 建立/更新 `ctl_products` 記錄
3. 呼叫 `saveTranslations()` 儲存商品翻譯
4. 處理商品選項：
   - update 時先刪除舊的 `product_options` + `product_option_values`
   - 逐筆建立 `ctl_product_options` 記錄
   - 若為選擇型：逐筆建立 `ctl_product_option_values` 記錄
...
刪除商品時，cascade 自動刪除：
- `ctl_product_translations`
- `ctl_product_options` → `ctl_product_option_values`
...
| 功能         | 資料表                 | 時機             |
|--------------|----------------------|------------------|
| 商品分類     | ctl_product_categories | Phase 1 後期     |
| 商品圖片     | ctl_product_images    | Phase 1 後期     |
| 折扣/特價    | ctl_product_discounts | Phase 2          |
| SEO URL      | ctl_product_seo_urls  | Phase 2          |
| 庫存關聯     | ctl_products.inv_item_id → inv_items | Phase 2（需 inv_ 子系統） |
| 品牌/製造商  | ctl_manufacturers     | Phase 2          |
| 商品屬性     | ctl_product_attributes | Phase 2         |
