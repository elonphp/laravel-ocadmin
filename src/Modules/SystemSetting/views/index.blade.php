...
@section('title', __('system-setting::setting.title'))
...
<div id="content">
    <div class="page-header">
        <div class="container-fluid">
            <div class="float-end">
                <button type="button" data-bs-toggle="tooltip" title="{{ __('ocadmin::common.filter') }}" onclick="$('#filter-setting').toggleClass('d-none');" class="btn btn-light d-lg-none">
                    <i class="fa-solid fa-filter"></i>
                </button>
                <a href="{{ ocadmin_route('settings.create') }}" data-bs-toggle="tooltip" title="{{ __('ocadmin::common.create') }}" class="btn btn-primary">
            ...
            <h1>{{ __('system-setting::setting.title') }}</h1>
            <ol class="breadcrumb">
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div id="filter-setting" class="col-lg-3 col-md-12 order-lg-last d-none d-lg-block mb-3">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form id="form-filter">
                            <div class="mb-3">
                                <label class="form-label">{{ __('system-setting::setting.code') }}</label>
                                <input type="text" name="filter_code" class="form-control"
                                       placeholder="{{ __('system-setting::setting.code') }}"
                                       value="{{ request('filter_code') }}">
                            </div>
                            <div class="mb-3">
                                <label class="form-label">{{ __('system-setting::setting.group') }}</label>
                                <input type="text" name="filter_group" class="form-control"
                                       placeholder="{{ __('system-setting::setting.group') }}"
                                       value="{{ request('filter_group') }}">
                            </div>
                            <div class="mb-3">
                                <label class="form-label">{{ __('system-setting::setting.type') }}</label>
                                <select name="filter_type" class="form-select">
                                    <option value="">-- {{ __('ocadmin::common.all') }} --</option>
                                    @foreach($types as $type)
                                    <option value="{{ $type->value }}" {{ request('filter_type') === $type->value ? 'selected' : '' }}>
                                        {{ $type->label() }}
                                    </option>
                                    @endforeach
                                </select>
            ...
            <div class="col-lg-9 col-md-12">
                <div class="card">
                    <div class="card-header">
                        <i class="fa-solid fa-list"></i> {{ __('system-setting::setting.list') }}
                    </div>
...
$(function() {
    ...
    $('#button-delete').on('click', function() {
        ...
        $.ajax({
            url: '{{ ocadmin_route("settings.destroy-multiple") }}',
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
                ids: selected
    ...
    function loadList() {
        var url = '{{ ocadmin_route("settings.list") }}';
