    ...
    <div class="page-header">
        <div class="container-fluid">
            <div class="float-end">
                ...
                <a href="{{ route('lang.ocadmin.system.access.user.create') }}" data-bs-toggle="tooltip" title="{{ $lang->button_add }}" class="btn btn-primary">
                    <i class="fa-solid fa-plus"></i>
                </a>
                <button type="button" id="button-delete" data-bs-toggle="tooltip" title="{{ $lang->button_remove }}" class="btn btn-danger">
...
$(document).ready(function() {
    ...
    $('#button-filter').on('click', function() {
        var url = '{{ route('lang.ocadmin.system.access.user.list') }}?';
        var params = [];
    ...
    $('#button-delete').on('click', function() {
        ...
        if (confirm('{{ $lang->text_confirm_remove }}')) {
            $.ajax({
                url: '{{ route('lang.ocadmin.system.access.user.batch-delete') }}',
                type: 'POST',
                data: {
                    selected: selected,
                    _token: '{{ csrf_token() }}'
                ...
                success: function(json) {
                    if (json.success) {
                        ...
                        alert(json.message || '{{ $lang->error_remove_failed }}');
                    }
                },
                error: function(xhr, ajaxOptions, thrownError) {
                    alert('{{ $lang->error_remove_failed }}：' + thrownError);
                }
            });
        }
    });
