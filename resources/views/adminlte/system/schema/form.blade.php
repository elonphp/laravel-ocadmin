...
<div class="app-content">
    <div class="container-fluid">
        ...
        <div id="alert-pending" class="alert alert-warning alert-dismissible fade" role="alert" style="display:none;">
            <i class="bi bi-exclamation-triangle"></i> {{ $lang->text_pending_changes }}
            <button type="button" class="btn btn-sm btn-success ms-2" id="button-apply-inline">
                <i class="bi bi-database"></i> {{ $lang->button_apply }}
            </button>
        </div>
        @endif
        <form action="{{ $is_new ? route('lang.ocadmin.system.schema.store') : route('lang.ocadmin.system.schema.update', $table_name) }}" method="post" id="form-schema" data-oc-toggle="ajax">
            ...
            {{-- 基本資訊 --}}
            <div class="card mb-3">
                <div class="card-header"><i class="bi bi-info-circle"></i> 基本資訊</div>
                <div class="card-body">
            ...
            <ul class="nav nav-tabs" id="schema-tabs">
                <li class="nav-item">
                    <a class="nav-link active" data-bs-toggle="tab" href="#tab-columns">
                        <i class="bi bi-layout-three-columns"></i> {{ $lang->tab_columns }}
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" data-bs-toggle="tab" href="#tab-translations">
                        <i class="bi bi-translate"></i> {{ $lang->tab_translations }}
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" data-bs-toggle="tab" href="#tab-indexes">
                        <i class="bi bi-list-ol"></i> {{ $lang->tab_indexes }}
                    </a>
                </li>
            </ul>
                    ...
                    <div class="card border-top-0 rounded-top-0">
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-bordered table-sm" id="table-columns">
                                    ...
                                    <tbody id="column-rows">
                                        ...
                                        <tr class="column-row">
                                            <td class="text-center sort-handle" style="cursor:grab;"><i class="bi bi-grip-vertical text-muted"></i></td>
                                            ...
                                            <td class="text-center"><button type="button" class="btn btn-danger btn-sm btn-remove-row"><i class="bi bi-x"></i></button></td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                            ...
                            <button type="button" id="button-add-column" class="btn btn-outline-primary btn-sm">
                                <i class="bi bi-plus-lg"></i> {{ $lang->button_add_column }}
                            </button>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card border-top-0 rounded-top-0">
                        <div class="card-body">
                            ...
                            <button type="button" id="button-add-translation" class="btn btn-outline-primary btn-sm">
                                <i class="bi bi-plus-lg"></i> {{ $lang->button_add_translation }}
                            </button>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card border-top-0 rounded-top-0">
                        <div class="card-body">
                            ...
                            <button type="button" id="button-add-index" class="btn btn-outline-primary btn-sm">
                                <i class="bi bi-plus-lg"></i> {{ $lang->button_add_index }}
                            </button>
                        </div>
                    </div>
                </div>
...
$(document).ready(function() {
    ...
    function doSync(source) {
        ...
        $.ajax({
            ...
            dataType: 'json',
            beforeSend: function() {
                $btn.prop('disabled', true).html('<i class="bi bi-arrow-repeat spinning"></i>');
            },
            success: function(json) {
                if (json.success) {
                    alert(json.message);
