...
<div class="app-content">
    <div class="container-fluid">
        {{-- 群組選擇 --}}
        <div class="card mb-3">
            <div class="card-header"><i class="bi bi-link"></i> {{ $lang->column_group }}</div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                <div class="card mb-3">
                    <div class="card-header"><i class="bi bi-diagram-3"></i> {{ $lang->text_link_setting }}</div>
                    <div class="card-body">
                        {{-- 層級切換頁籤 --}}
            ...
            <div class="col-lg-4">
                <div class="card mb-3">
                    <div class="card-header"><i class="bi bi-funnel"></i> {{ $lang->text_cascade_test }}</div>
                    <div class="card-body">
                        @foreach($groupData['levels'] as $i => $level)
                        <div class="mb-3">
        ...
        <div class="card">
            <div class="card-body text-center text-muted py-5">
                ...
                <p class="mt-3">{{ $lang->text_no_group }}</p>
            </div>
        </div>
        @endif
    </div>
...
function selectParent(parentId, level) {
    ...
    $.getJSON(url_links.replace('__ID__', parentId), function(data) {
        ...
        html += '<div class="mt-3"><button type="button" class="btn btn-primary btn-sm" onclick="saveLinks(' + parentId + ', ' + level + ')"><i class="bi bi-floppy"></i> {{ $lang->button_save }}</button> <span id="save-status-' + level + '"></span></div>';
...
function saveLinks(parentId, level) {
    ...
    $.ajax({
        ...
        success: function(json) {
            if (json.success) {
                $('#save-status-' + level).html('<span class="text-success"><i class="bi bi-check"></i> {{ $lang->text_saved }}</span>');
