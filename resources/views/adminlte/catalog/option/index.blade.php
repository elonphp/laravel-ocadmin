...
<div class="app-content">
    <div class="container-fluid">
        ...
        <div class="row">
            {{-- 篩選區塊 --}}
            <div id="filter-option" class="col-lg-3 col-md-12 order-lg-last d-none d-lg-block mb-3">
                <div class="card">
                        ...
                        <form id="form-filter">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="equal_type" id="input-type" class="form-select">
                                    ...
                                    <option value="text" {{ request('equal_type') === 'text' ? 'selected' : '' }}>{{ $lang->text_text }}</option>
                                    <option value="textarea" {{ request('equal_type') === 'textarea' ? 'selected' : '' }}>{{ $lang->text_textarea }}</option>
                                    <option value="file" {{ request('equal_type') === 'file' ? 'selected' : '' }}>{{ $lang->text_file }}</option>
                                    <option value="date" {{ request('equal_type') === 'date' ? 'selected' : '' }}>{{ $lang->text_date }}</option>
                                    <option value="time" {{ request('equal_type') === 'time' ? 'selected' : '' }}>{{ $lang->text_time }}</option>
                                    <option value="datetime" {{ request('equal_type') === 'datetime' ? 'selected' : '' }}>{{ $lang->text_datetime }}</option>
                                </select>
                            </div>
                            <div class="text-end">
            ...
            <div class="col-lg-9 col-md-12">
                <div class="card">
                    ...
                    <div id="option-list" class="card-body">
                        {!! $list !!}
                    </div>
                </div>
            </div>
...
$(document).ready(function() {
    $('#option-list').on('click', 'thead a, .pagination a', function(e) {
        e.preventDefault();
        $('#option-list').load($(this).attr('href'));
    });
    ...
    $('#button-filter').on('click', function() {
        ...
        params.push('equal_type=' + encodeURIComponent($('#input-type').val()));

        url += params.join('&');
        window.history.pushState({}, null, url.replace('/list?', '?'));
        $('#option-list').load(url);
    });
